...
ALTER TABLE IF EXISTS public.inventory
    ...
    -- It means that the values in the "classification_id" column of the "inventory" table must correspond to values in the "classification_id" column of another table.
    REFERENCES public.classification (classification_id) MATCH SIMPLE 
    -- "MATCH SIMPLE" means that the foreign key values don't need to match exactly. It's the default behavior.
    ON UPDATE CASCADE
    -- "ON UPDATE CASCADE" means that if a classification ID is updated in the "classification" table, the corresponding values in the "inventory" table will also be updated automatically.
    ON DELETE NO ACTION;
    -- "ON DELETE NO ACTION" means that no action will be taken in the "inventory" table if a corresponding classification is deleted.
